...
## 2026-09-18 (same session) — card RE.3, redesigned on Jonas's question about the normative model: the boundary is NEITHER purely perceptual NOR configurational

Jonas: *"is this not a lot about the normative model? If people are expecting and accepting for
example the cut-in to have a closing relative speed, as long as it is far away."* And: *"What is
RE.3 test that you want to run?"*

**The RE.3 in `docs/active_inference_reformulation.md` §6 was wrong and is replaced.** As written it
was to fit the threshold model on a theta_dot preference factor — which would have reproduced card
EL.1b's own 0.1137 by construction, because a preference on theta_dot IS the looming axis. That is
a re-labelling, not a test. Jonas was right to push. The note's §6 is rewritten.

**Why the normative question is the right one, and what it can and cannot decide.** The R.2 pipeline
review already named this as the crux — "every surprise measure needs a reference distribution, and
the field was ours" (§5) — and set two constraints on any candidate reference: it must engage at
encroachment onset **independent of the lateral pace** (the response is flat across lane-change
duration at matched TTC and dv), and **within matched TTC it must order by gap, not by absolute
speed**. The first of those is important for Jonas's hypothesis and sharpens it: the norm that
matters cannot be about the *execution* of the manoeuvre (abruptness), because abruptness does not
move the response. It has to be about the **configuration** — which is exactly his wording, a
closing speed being acceptable as long as it is far away.

On the second cut-in study the normative and perceptual accounts are **observationally identical**:
both produce a curve in the (gap, closing speed) plane. So that study cannot separate them, and no
amount of further work on it will.

**RE.3, redesigned: the apparent-size discriminator** (`replication/czb/re3_apparent_size_discriminator.py`
→ `out/re3_apparent_size.md`, `.csv`). One design in this project CAN separate them and the data are
in hand: the truck and car cut-ins of study 1 at matched TTC. A truck is 2.55 m wide against a car's
1.88 m (both measured from the traces), so at the same configuration it expands faster. Writing the
criterion as a constant value of `W^a dv / gap^2`, the class offset measured on log theta_dot is
exactly `(1 - a) log(W_truck / W_car)`:

- **perceptual** (a = 1, a threshold on what the eye receives): offset **0**
- **configurational** (a = 0, width-blind, "far away is fine"): offset **+0.304**

Measured, on card JJ.4's press levels (3 417 trials, 43 drivers, TTC 4 to 8), by a driver-clustered
difference with a 2 000-resample bootstrap over drivers:

| | value |
|---|---|
| class offset (truck - car) on log theta_dot | **-0.1285** [-0.1922, -0.0721] |
| card JJ.4's hierarchical model, same data | -0.1265 |
| implied width exponent a | **1.42** [1.24, 1.63] |

**The interval excludes BOTH predictions, so the pre-stated verdict is (N) NEITHER.** And the sign
is the informative part: the offset is on the far side of the perceptual prediction from the
configurational one. The criterion weights apparent size **more** heavily than optical expansion
alone — a driver treats a wide vehicle as closer to their boundary than its expansion rate says.
A width-blind configuration norm is the reading this rules out most firmly: it is four standard
errors away in the wrong direction.

Stated in the report and worth repeating: a truck differs from a car in more than width (16.5 m
long against about 5 m), so this bounds how much of the criterion is optical without proving that
the remainder is width specifically.

**The part that needs the naturalistic data, now registered rather than left open.** Card EL.1's
full-sample w = 0.497 on the linear rule puts the comfort boundary's slope in the (log gap, log dv)
plane at **1.988**, against 2 for constant looming, 1 for constant TTC and infinity for constant
gap. **The pre-registered prediction is that the iso-density contours of p(gap, dv | a lane change
into my lane) in naturalistic data have that slope.** If they do, the normative and perceptual
accounts coincide on this scenario and the project should stop trying to separate them there; if
they do not, the difference between the slopes is the size of the normative contribution. This
turns the naturalistic data request into a test with a number attached instead of an exploration.

Queries:

@RE3.Q1(judgment, jonas): the implied width exponent is 1.42, not 1. Two readings this card cannot
separate: drivers weight a large vehicle as more threatening than its optics (a mass or severity
expectation), or the truck's length and lane-change kinematics carry the difference and width is
the wrong label for it. The first is a comfort-zone finding; the second is a confound. Separating
them needs a stimulus set that varies width and length independently, which this study does not
have. Worth a data request, or park it?

@RE3.Q2(judgment, review): the slope prediction of 1.988 is registered here against naturalistic
cut-in data that the project does not have. When the data arrive, is the right comparison the
