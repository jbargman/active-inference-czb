...
description: How research work is executed for Jonas Bärgman — the interruption budget and the two operating modes (interactive versus batch/overnight), the @REVIEW and @JONAS flagging convention that lets a session continue instead of blocking, reproducibility rules (every quoted number from a committed script), verification and test discipline, the rule that every parameter value carries a motivation, replication of published results when a component comes from a paper, documentation and handover conventions, model-tier awareness for deciding who a question can be deferred to, and the statistical conventions. Use when executing any research or analysis task, when starting or ending a work session, when writing or reading a handover, when a result is about to be quoted, and when work passes between models or sessions.
---
...
The resolution is **flag and continue, do not stop and ask**. Nearly every question that
would justify interrupting can instead be recorded, tagged, and carried forward — the
work continues, and he reads the tags when he is next available.

## 2 Two modes

**Interactive mode** — he is present and responding. Ask when a decision is genuinely
his, but prefer a tagged note plus a stated default over a blocking question.
...
- A failed acceptance criterion, a contradiction, a stop condition, a judgment call —
  all are **recorded and tagged**, and the session moves to the next item.
...
action, tag it, continue the queue. This is not a blocking stop — the queue keeps
moving; only that one action waits.

## 3 The flagging convention

Two tags, both greppable, both appearing in the per-item log entry **and** in the
end-of-session summary:

- `@REVIEW` — a more capable model should assess this: a judgment call, a verdict, a
  contradiction with documented findings, an interpretation that is not yet checked.
- `@JONAS` — only Jonas can settle this: scope, authorization, a preference, anything
  outward-facing, or a parameter whose motivation is weak (see §5).

Each carries a severity:

- `(blocker)` — later work depends on it and may be built on sand.
- `(judgment)` — a defensible choice was made; someone should confirm it.
- `(minor)` — worth knowing, nothing waits on it.

Format, one line, greppable:

```
@REVIEW(blocker): card A.3 — accumulator gain hit its bound; fit reported at the bound.
@JONAS(judgment): card B.1 — cyclist width taken as 0.6 m, no source found in the data.
```

Retrieval: `grep -rn "@REVIEW\|@JONAS" <repo>/replication/*/out/worklog.md`

**A tag is not a substitute for doing the work.** Tag *and* proceed with the best
defensible choice, stating what was chosen. A tag that says "did not attempt" is only
...
- [ ] For each item: execute, check acceptance criteria, tag anything unresolved
...
- [ ] End-of-session summary: what was done, per item, and every tag repeated
```
...
- **If the motivation is weak, tag `@JONAS(judgment)`.** Weak means: it works but you
...
- **A result contradicting a documented finding is surfaced, never silently
  reconciled**: name the document, quote the disagreement, tag `@REVIEW`.
...
**If you are tier 1, there is nobody above you.** `@REVIEW` does not summon a better
model; it only reaches Jonas, whose time is the thing being protected. Therefore:
resolve what you can rather than deferring it, use `@JONAS` for what genuinely needs
him, and **self-review before finishing** — re-derive your own quoted numbers from their
committed scripts, since no stronger reviewer follows you. Review gates should be run at
tier 1 where possible.

**If you are tier 2 or below**, `@REVIEW` is a real escalation path: tag the judgment
call, make the best defensible choice, and keep going.

## 11 What warrants a tag
...
1. **May an executing session write interpretation?** The old work-order rule said "no
   ...
   it `@REVIEW(judgment)` as unreviewed, so a later reader knows it has not been
   checked. Only designated verdict cards keep the stricter "no verdict prose" rule.
...
across a real overnight batch yet. If the tag volume turns out wrong in either
direction — too many `@JONAS(minor)` entries to read, or blockers buried among them —
the severity thresholds are the thing to tune first.
