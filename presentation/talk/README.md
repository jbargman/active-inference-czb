...
| `ai_czb_concepts_talk-v7.pptx` | **current.** v6 plus **slide 3, "What a vehicle would actually do with this"** (Jonas: the model predicts a share, but what would the ADAS use?): FIT → INVERT → CHOOSE → TRIGGER, with the percentile named as the policy step and a caveat strip saying this is a method for setting a threshold, not a fleet trigger. 19 slides, 12 animated, 28.8 min. Every later slide number shifts by one |
