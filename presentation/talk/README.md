...
| `ai_czb_concepts_talk-v8.pptx` | **current.** v7 with three explanation fixes, no new slides: the AXIS slide's on-slide caption now names the trap outright (the top panel separates the stimuli dramatically, participants treated them as alike, so it discriminates something people did not use) and its notes answer the objection head-on; the GATE and NOISE FLOOR notes are substantially expanded on Jonas's "I am not sure I understand it". 19 slides, 12 animated, 29.1 min |
| `ai_czb_concepts_talk-v7.pptx` | superseded. v6 plus **slide 3, "What a vehicle would actually do with this"** (Jonas: the model predicts a share, but what would the ADAS use?): FIT → INVERT → CHOOSE → TRIGGER, with the percentile named as the policy step and a caveat strip saying this is a method for setting a threshold, not a fleet trigger. 19 slides, 12 animated, 28.8 min. Every later slide number shifts by one |
| `ai_czb_concepts_talk-v6.pptx` | superseded. v5 with the video-content bug fixed: every `.mp4` was a SECOND render of the same `FuncAnimation`, replayed with the artists still holding the first pass's final state, so each video began with the whole animation already drawn. The `.mp4` and the poster are now derived from the finished `.gif` (`gif_to_mp4`), so still, video and animation cannot disagree. Also: the axis slide's right panel now says it is raw response rates with no model. 18 slides, 12 animated, 27.2 min |
| `ai_czb_concepts_talk-v5.pptx` | superseded. v4 with Jonas's second review applied: new title ("Yet another modeling perspective" / "Starting with active inference, developing into a gated threshold model of the comfort-zone boundary: the concepts one at a time") with the vocabulary sprinkled below; Sarang named on the motivation slide; the trait animation embedded as video and its y-limits taken from the data so the topmost driver no longer runs off the top; **posters are now the FIRST frame**, so playing a video no longer wipes the still and rebuilds it. 18 slides, 12 animated, notes budget 27.2 min |
...
`save()` in `make_concept_animations.py` renders the `.gif` ONCE and then derives the other
two from it with `gif_to_mp4`. Never render an animation twice from the same `FuncAnimation`:
these frame functions only add to their artists, so the second pass starts with the first
pass's finished picture already on screen. `save()` writes all three (`.gif`, `.mp4`, `.png`)
