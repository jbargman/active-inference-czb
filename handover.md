...
3. **Human data acquisition for the CZB transfer test** — still the project's decisive
   scientific step (HANDOFF §6 item 1; `docs/data_requirements.pdf`).
4. **The a_f,min lever**: every configuration fails the braking-distribution metric; the
   unpulled lever is the execution model (the CBM jerk-ramp is shared by both response
   processes; tier-2 endogenous braking or a refined execution model would address it).
